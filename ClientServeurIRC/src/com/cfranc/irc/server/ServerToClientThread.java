...
public class ServerToClientThread extends Thread implements IfClientServerProtocol{
	private User user;
				...
				try {
					if(streamIn.available()>0){
						...
						if (line.startsWith(DEL)) {
							String[] userMsg =line.substring(DEL.length()) .split(IfClientServerProtocol.SEPARATOR);
							User deletedUser = new User(userMsg[0], "");
							BroadcastThread.removeClient(deletedUser);
						} else  {
